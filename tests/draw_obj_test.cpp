...
void
def_vulkan(BfBase& base, BfHolder& holder)
{
   bfBindHolder(&holder);
...
TEST_CASE("bdDrawObject-x2")
{
   ...
   void* pdata =
       layer.__buffer.get_vertex_allocation_info()
           .pMappedData;  //__base.descriptor.get_buffer(BfDescriptorPosPickUsage,
                          //__base.current_frame)->allocation_info.pMappedData;
...
/*
        ...
        info_mtx.shader_stages = VK_SHADER_STAGE_VERTEX_BIT |
   VK_SHADER_STAGE_FRAGMENT_BIT; info_mtx.binding = 1;
   ...
   VK_SHADER_STAGE_FRAGMENT_BIT; info_view.binding = 0;
