...
BfGuiSmartLayerObserver::BfGuiSmartLayerObserver()
    : BfGuiWindow{
          {.name = "Smart layer observer",
           .isRender = true,
           .toggleRenderMapping = {ImGuiKey_LeftCtrl, ImGuiKey_T}}
      }
{
}
...
)
{
   if (ImGui::BeginDragDropTarget())
   {
              ...
              ImGui::AcceptDragDropPayload("Moving draw layer"))
      {
         ...
         std::cout << "DONE->" << input_l->id.get() << "\n";
         __current_moving_id = 0;
      }
      ImGui::EndDragDropTarget();
   }
   // else if (__current_moving_id == l->id.get())
   // {
   //    __current_moving_id = 0;
   // }
}
