...
BfBladeSection::__generate_outer_elements()
{
   ...
   //     glm::normalize(glm::cross(__info->up_direction,
   //     __info->grow_direction));
...
BfBladeSection::__generate_average_line_geometry()
{
   ...
   //     (get_top_border()->get_first().pos +
   //     get_bot_border()->get_first().pos) / 2.0f;
   //
...
BfBladeSection::__generate_obj_frames()
{
   ...
   // else
   // {
   //    get_ave_curve_frame()->remake(get_ave_curve(),
   //    BF_BLADESECTION_AVE_COLOR); get_back_curve_frame()->remake(
   //        get_back_curve(),
