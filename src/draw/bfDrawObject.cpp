...
BfDrawObj::BfDrawObj() : BfDrawObj(0) {}

BfDrawObj::BfDrawObj(uint32_t type) : id(type) {}
...
BfObjID::BfObjID() : BfObjID(0) {}

BfObjID::BfObjID(uint32_t type) : __type{type} {
...
BfDrawLayer::BfDrawLayer(VmaAllocator allocator, size_t vertex_size,
                         size_t max_vertex_count, size_t max_reserved_count,
                         bool is_nested)
...
BfDrawLayer::BfDrawLayer(const BfDrawLayerCreateInfo &info)
    : BfDrawLayer(info.allocator, info.vertex_size, info.max_vertex_count,
                  info.max_reserved_count, info.is_nested) {}
...
void BfDrawLayer::update_nested(void *v, void *i, size_t &off_v,
                                size_t &off_i) {
  if (!is_nested())
    throw std::runtime_error("layer is not nested");
  if (v == nullptr)
    throw std::runtime_error("vertex* = nullptr");
  if (i == nullptr)
    throw std::runtime_error("index* = nullptr");
...
void BfDrawLayer::draw(VkCommandBuffer combuffer, size_t &offset,
  ...
  if (!is_nested()) {
    ...
    vkCmdBindVertexBuffers(combuffer, 0, 1, __buffer.get_p_vertex_buffer(),
                           vert_offset.data());
    vkCmdBindIndexBuffer(combuffer, *__buffer.get_p_index_buffer(), 0,
  ...
  for (size_t i = 0; i < __objects.size(); i++) {
    if (__objects[i] == nullptr)
      continue;
    if (__objects[i]->get_bound_pPipeline() == nullptr) {
      throw std::runtime_error("Object pipeline pointer is nullptr");
    }
    if (!__objects[i]->is_draw)
      continue;
    vkCmdBindPipeline(combuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                      *__objects[i]->get_bound_pPipeline());
    vkCmdDrawIndexed(combuffer, __objects[i]->get_indices_count(), 1,
                     __index_offsets[i] + index_offset,
                     __vertex_offsets[i] + vertex_offset, i + offset);
  }
  if (!__objects.empty()) {
    vertex_offset +=
