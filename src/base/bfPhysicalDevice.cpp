...
//static std::map<BfvEnQueueType, std::optional<uint32_t>> queue_family_indices;

// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Static definitions //

BfEvent bfIsQueueFamilyIndicesCorrect(const BfPhysicalDevice* bf_physical_device, bool& is_suitable)
{
	...
	if (decision) {
		...
		event.action = BfEnActionType::BF_ACTION_TYPE_SEARCH_NEEDED_QUEUES_SUCCESS;
	}
	else
	{
		event.type = BfEnSingleEventType::BF_SINGLE_EVENT_TYPE_CHECK_EVENT;
		event.action = BfEnActionType::BF_ACTION_TYPE_SEARCH_NEEDED_QUEUES_FAILURE;
	}
...
BfEvent bfCheckDeviceExtensionSupport(BfPhysicalDevice* physical_device, bool& is_suitable)
{
	...
	vkEnumerateDeviceExtensionProperties(physical_device->physical_device, nullptr,
	...
	std::set<std::string> requiredExtensions(bfvDeviceExtensions.begin(), 
	...
	if (is_suitable) {
		event.type = BfEnSingleEventType::BF_SINGLE_EVENT_TYPE_CHECK_EVENT;
		event.action = BfEnActionType::BF_ACTION_TYPE_CHECK_EXTENSION_SUPPORT_SUCCESS;
	}
	else {
		event.type = BfEnSingleEventType::BF_SINGLE_EVENT_TYPE_CHECK_EVENT;
		event.action = BfEnActionType::BF_ACTION_TYPE_CHECK_EXTENSION_SUPPORT_FAILURE;
	}
	return BfEvent(event);
}
...
BfEvent bfFindQueueFamilyIndices(BfPhysicalDevice* bf_physical_device, VkSurfaceKHR surface)
{
	...
	vkGetPhysicalDeviceQueueFamilyProperties(bf_physical_device->physical_device, &queueFamiliesCount, nullptr);

	std::vector<VkQueueFamilyProperties> queueFamilies(queueFamiliesCount);
	vkGetPhysicalDeviceQueueFamilyProperties(bf_physical_device->physical_device, &queueFamiliesCount, queueFamilies.data());
	...
	for (const auto& queueFamily : queueFamilies) {
		...
		if (bfvEnabledQueueTypes.contains(BF_QUEUE_PRESENT_TYPE))
		{
			...
			vkGetPhysicalDeviceSurfaceSupportKHR(bf_physical_device->physical_device, i, surface, &presentSupport);
...
BfEvent bfIsDeviceSuitable(BfPhysicalDevice* bf_physical_device, VkSurfaceKHR surface, bool& is_suitable)
{
	...
	if (extensionsSupport) {
		...
		bfGetSwapChainSupport(bf_physical_device->physical_device, surface, swapChainSupport);
	...
	if (isQueuesCorrect && extensionsSupport && swapChainAdequate) {
		is_suitable = true;
		event.type = BfEnSingleEventType::BF_SINGLE_EVENT_TYPE_CHECK_EVENT;
		event.action = BfEnActionType::BF_ACTION_TYPE_CHECK_PHYSICAL_DEVICE_SUITABILITY_SUCCESS;
	}
	else {
		is_suitable = false;
		event.type = BfEnSingleEventType::BF_SINGLE_EVENT_TYPE_CHECK_EVENT;
		event.action = BfEnActionType::BF_ACTION_TYPE_CHECK_PHYSICAL_DEVICE_SUITABILITY_FAILURE;
	}
