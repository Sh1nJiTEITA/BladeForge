...
BfEvent bfCreateSurface(BfBase &base) {
  BfSingleEvent event{};
  if (glfwCreateWindowSurface(base.instance, base.window->pWindow, nullptr,
...
BfEvent bfCreateLogicalDevice(BfBase &base) {
  ...
  BfSingleEvent event{};
  if (vkCreateDevice(base.physical_device->physical_device, &createInfo,
                     nullptr, &base.device) != VK_SUCCESS) {
    event.type = BfEnSingleEventType::BF_SINGLE_EVENT_TYPE_INITIALIZATION_EVENT;
  ...
  for (auto &it : base.physical_device->queue_family_indices) {
    vkGetDeviceQueue(base.device, it.second.value(), 0,
...
BfEvent bfCreateSwapchain(BfBase &base) {
  ...
  */
  ...
  BfSingleEvent event{};
  if (vkCreateSwapchainKHR(base.device, &createInfo, nullptr,
  ...
  vkGetSwapchainImagesKHR(base.device, base.swap_chain, &imageCount,
...
BfEvent bfCreateImageViews(BfBase &base) {
  ...
  for (size_t i = 0; i < holder->image_views.size(); i++) {
    ...
    BfSingleEvent event{};
    if (vkCreateImageView(base.device, &createInfo, nullptr,
...
BfEvent bfDestroyImageViews(BfBase &base) {
  ...
  for (size_t i = 0; i < holder->image_views.size(); i++) {
    vkDestroyImageView(base.device, *base.image_packs[i].pImage_view, nullptr);
  }
...
BfEvent bfCreateStandartRenderPass(BfBase &base) {
  ...
  std::vector<VkAttachmentDescription> attachments{
      colorAttachment, id_map_attachment, depth_attachment};

  std::vector<VkSubpassDependency> dependencies{dependency, id_dependency,
                                                depth_dependency};
  ...
  BfSingleEvent event{};
  if (vkCreateRenderPass(base.device, &renderPassInfo, nullptr,
...
BfEvent bfInitOwnDescriptors(BfBase &base) {
  ...
  id_image_create_info.extent = {base.swap_chain_extent.width,
                                 base.swap_chain_extent.height, 1};
  ...
  bfCreateBuffer(&base.id_image_buffer, base.allocator,
...
void bfPopulateMessengerCreateInfo(
  ...
  createInfo.messageSeverity = {
      VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT | // Just info (e.g. creation
                                                     // info)
  ...
  createInfo.messageType = {
      VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | // Unrelated perfomance (mb
                                                    // ok)
      VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT | // Possiable mistake (not
                                                       // ok)
...
BfEvent bfCreateGraphicsPipelines(BfBase &base) {
  ...
  bfaCreateShaderCreateInfos(base.device, "shaders/basic", modules_basic,
                             infos_basic);

  std::vector<VkShaderModule> modules_tlines;
  std::vector<VkPipelineShaderStageCreateInfo> infos_tlines;
  bfaCreateShaderCreateInfos(base.device, "shaders/thick_lines", modules_tlines,
                             infos_tlines);

  std::vector<VkShaderModule> modules_lines;
  std::vector<VkPipelineShaderStageCreateInfo> infos_lines;
  bfaCreateShaderCreateInfos(base.device, "shaders/lines", modules_lines,
                             infos_lines);
  ...
  std::vector<VkPipelineColorBlendAttachmentState> colorBlendAttachments = {
      colorBlendAttachment, idBlendAttachment};
  ...
  if (vkCreateGraphicsPipelines(base.device, VK_NULL_HANDLE, 1, &pipelineInfo,
                                nullptr,
  ...
  if (vkCreateGraphicsPipelines(base.device, VK_NULL_HANDLE, 1, &pipelineInfo,
                                nullptr, &base.tline_pipeline) != VK_SUCCESS) {
    is_pipelines_created = false;
  ...
  if (vkCreateGraphicsPipelines(base.device, VK_NULL_HANDLE, 1, &pipelineInfo,
                                nullptr, &base.line_pipeline) != VK_SUCCESS) {
    is_pipelines_created = false;
...
BfEvent bfCreateStandartFrameBuffers(BfBase &base) {
  ...
  for (size_t i = 0; i < base.image_pack_count; i++) {
    ...
    std::vector<VkImageView> attachments = {
        *base.image_packs[i].pImage_view, *base.image_packs[i].pImage_view_id,
    ...
    if (vkCreateFramebuffer(base.device, &framebufferInfo, nullptr,
...
BfEvent bfDestroyStandartFrameBuffers(BfBase &base) {
  ...
  for (uint32_t i = 0; i < base.image_pack_count; i++) {
    vkDestroyFramebuffer(base.device, holder->standart_framebuffers[i],
                         nullptr);
  }
...
BfEvent bfCreateGUIFrameBuffers(BfBase &base) {
  ...
  for (uint32_t i = 0; i < base.image_pack_count; i++) {
    ...
    if (vkCreateFramebuffer(base.device, &info, nullptr,
...
BfEvent bfCreateCommandPool(BfBase &base) {
  ...
  BfSingleEvent event{};
  if (vkCreateCommandPool(base.device, &poolInfo, nullptr,
...
BfEvent bfCreateStandartCommandBuffers(BfBase &base) {
  ...
  if (vkAllocateCommandBuffers(base.device, &allocInfo,
...
BfEvent bfDestroyStandartCommandBuffers(BfBase &base) {
  ...
  vkFreeCommandBuffers(base.device, base.command_pool,
...
BfEvent bfDestorySyncObjects(BfBase &base) {
  auto holder = bfGetpHolder();
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    vkDestroySemaphore(base.device, holder->available_image_semaphores[i],
                       nullptr);
    vkDestroySemaphore(base.device, holder->finish_render_image_semaphores[i],
                       nullptr);
...
BfEvent bfCreateDepthBuffer(BfBase &base) {
  ...
  VkImageCreateInfo dimg_info = bfPopulateDepthImageCreateInfo(
      base.depth_format, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
      depthImageExtent);
  ...
  auto res = vmaCreateImage(base.allocator, &dimg_info, &dimg_allocinfo,
  ...
  VkImageViewCreateInfo dview_info = bfPopulateDepthImageViewCreateInfo(
      base.depth_format, base.depth_image.image, VK_IMAGE_ASPECT_DEPTH_BIT);

  BfSingleEvent event{};
  if (vkCreateImageView(base.device, &dview_info, nullptr,
...
BfEvent bfCreateIDMapImage(BfBase &base) {
  ...
  bool success = 1;
  for (int frame_index = 0; frame_index < base.image_pack_count;
       frame_index++) {
    auto image_event =
        bfCreateImage(&holder->images_id[frame_index], base.allocator,
                      &id_image_create_info, &allocinfo);
    id_image_info.image = holder->images_id[frame_index].image;

    auto view_event = bfCreateImageView(&holder->images_id[frame_index],
                                        base.device, &id_image_info);
...
void bfUploadVertices(BfBase &base, BfMesh &mesh) {
  ...
  if (vmaCreateBuffer(base.allocator, &bufferInfo, &allocationInfo,
                      &local_buffer, &local_allocation,
                      nullptr) != VK_SUCCESS) {
  ...
  vkCmdCopyBuffer(commandBuffer, local_buffer, mesh.vertex_buffer.buffer, 1,
                  &copyRegion);
  ...
  vkQueueSubmit(
      base.physical_device->queues[BfvEnQueueType::BF_QUEUE_GRAPHICS_TYPE], 1,
...
void bfUploadIndices(BfBase &base, BfMesh &mesh) {
  ...
  copyRegion.dstOffset = 0; // Optional
  copyRegion.size = bufferInfo.size;
  vkCmdCopyBuffer(commandBuffer, local_buffer, mesh.index_buffer.buffer, 1,
                  &copyRegion);
  vkEndCommandBuffer(commandBuffer);
...
BfEvent bfaCreateGraphicsPipelineLayouts(BfBase &base) {
  ...
  if (vkCreatePipelineLayout(base.device, &pipelineLayoutCreateInfo, nullptr,
...
void bfDrawFrame(BfBase &base) {
  ...
  VkResult result = vkAcquireNextImageKHR(
      base.device, base.swap_chain, UINT64_MAX, local_available_image_semaphore,
      VK_NULL_HANDLE, &base.current_image);
  ...
  if (vkQueueSubmit(
          base.physical_device->queues[BfvEnQueueType::BF_QUEUE_GRAPHICS_TYPE],
          1, &submitInfo, local_fence_in_flight) != VK_SUCCESS) {
...
void bfMainRecordCommandBuffer(BfBase &base) {
  ...
  vkCmdBeginRenderPass(local_buffer, &renderPassInfo,
                       VK_SUBPASS_CONTENTS_INLINE);
  {
    ...
    base.descriptor.bind_desc_sets(
        BfEnDescriptorSetLayoutType::BfDescriptorSetGlobal, base.current_frame,
        local_buffer, base.triangle_pipeline_layout, 0);
    base.descriptor.bind_desc_sets(
        BfEnDescriptorSetLayoutType::BfDescriptorSetMain, base.current_frame,
        local_buffer, base.triangle_pipeline_layout, 1);
...
void bfUpdateUniformBuffer(BfBase &base) {
  ...
  if (base.window->proj_mode == 0) {
    ...
    if (base.window->is_asp) {
      float asp = (float)base.swap_chain_extent.height /
                  (float)base.swap_chain_extent.width;
      ubo.proj = glm::ortho(base.window->ortho_left, base.window->ortho_right,
                            base.window->ortho_bottom * asp,
                            base.window->ortho_top * asp,
                            base.window->ortho_near, base.window->ortho_far);
    } else {
      ubo.proj = glm::ortho(base.window->ortho_left, base.window->ortho_right,
                            base.window->ortho_bottom, base.window->ortho_top,
                            base.window->ortho_near, base.window->ortho_far);
    }
    ubo.model = glm::scale(glm::mat4(1.0f),
                           {base.window->ortho_scale, base.window->ortho_scale,
  ...
  base.descriptor.map_descriptor(BfDescriptorViewDataUsage, base.current_frame,
                                 &view_data);
