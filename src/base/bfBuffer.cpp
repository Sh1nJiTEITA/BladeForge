...
BfEvent bfDestroyImage(BfAllocatedImage* allocatedImage)
{
   ...
   vmaDestroyImage(allocatedImage->allocator,
                   allocatedImage->image,
                   allocatedImage->allocation);
    ...
    , __is_nested{is_nested}
{
   if (!__is_nested)
   {
      ...
      size_t index_size =
          (size_t)(sizeof(uint32_t) * max_vertex_count * max_obj_count * 1.2);
      BfEvent i_event =
...
bool BfLayerBuffer::is_nested() const noexcept { return __is_nested; }
...
{
   vmaMapMemory(__vertex_buffer.allocator,
                __vertex_buffer.allocation,
                &__p_vertex_data);
...
{
   vmaMapMemory(__index_buffer.allocator,
                __index_buffer.allocation,
                &__p_index_data);
...
VkBuffer* BfLayerBuffer::get_p_index_buffer() { return &__index_buffer.buffer; }
