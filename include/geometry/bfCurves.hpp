...
public:
   ...
   BfHandle()
       : BfHandle(BfVertex3(), 1.0f, {1.0f, 0.0f, 1.0f})
   {
   }
...
public:
    ...
    */
   BfBezier()
       : n{0}, vertices{}, c_vertices{}, basic_color{1.0f, 1.0f, 1.0f}
   {
   }
   ...
   glm::vec3 get_single_derivative_1(float t) const
   {
      ...
      for (int i = 0; i <= k; i++)
      {
         out += (float)(bfGetBinomialCoefficient(k, i) *
                        glm::pow(1 - t, k - i) * glm::pow(t, i) * n) *
   ...
   glm::vec3 get_single_derivative_1_e(float t)
   {
      ...
      for (int i = 0; i <= n; i++)
      {
         out += (float)((i - n * t) / (t * (1 - t)) *
                        bfGetBinomialCoefficient(n, i) *
   ...
   // o.vertices[i].y, o.vertices[i].z, 1.0f };
   ...
   ) const
   {
      ...
      for (size_t i = 0; i < this->vertices.size(); i++)
      {
         ...
         std::copy(
             handle.get_vertices().begin(),
             handle.get_vertices().end(),
   ...
   )
   {
      ...
      {
         throw std::runtime_error(
             "intesections can't be found: no calculated points in one of "
             "curves"
         );
      }
...
private:
   ...
   {
      return glm::cross(
          this->get_direction_normal(0.5f),
...
inline uint32_t
bfGetBinomialCoefficient(uint32_t n, uint32_t k)
{
