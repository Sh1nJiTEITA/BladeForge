...
public:
  ...
  void update_vertices() {
    glm::vec3 v1 = glm::normalize(
        glm::cross(__center.normals, glm::vec3(1.0f, 0.0f, 0.0f)));
...
/*
 * BfLine class provides opportunity to work with lines defined by 2 glm::vec3
 * points.
 */
class BfLine {
...
public:
   ...
   *			const BfLine& line1 - const reference to 1st line to
   *work with const BfLine& line2 - const reference to 2st line to work with
   */
  static inline bool find_lines_intersection(glm::vec3 &intersection,
                                             const BfLine &line1,
                                             const BfLine &line2) {
    glm::mat3 plane;
  ...
  static inline float get_line_length(const BfLine &l) {
    ...
    return glm::sqrt(delta.x * delta.x + delta.y * delta.y + delta.z * delta.z);
  }
};
...
public:
  ...
  BfBezier(uint32_t in_n, std::vector<BfVertex3> &in_vertices,
  ...
  BfBezier(uint32_t in_n, std::vector<glm::vec3> in_vertices,
           glm::vec3 in_color = {1.0f, 1.0f, 1.0f})
   ...
   *
   * Needed: 1. t - relative Bezier curve parameter, standart value of t = [0,
   * 1]
   *
   */
  glm::vec3 get_single_vertex(float t) const {
    ...
    for (int i = 0; i <= n; i++) {
      _v += static_cast<float>(bfGetBinomialCoefficient(n, i) *
                               glm::pow(1 - t, n - i) * glm::pow(t, i)) *
            vertices[i];
    }
    return _v;
  }
   ...
   *
   */
  glm::vec3 get_tangent(float t) { return this->get_single_derivative_1(t); }
  ...
  glm::vec3 get_direction_tangent(float t) const {
    ...
    float d = glm::sqrt(tangent.x * tangent.x + tangent.y * tangent.y +
                        tangent.z * tangent.z);
  ...
  glm::vec3 get_single_derivative_1(float t) const {
    ...
    glm::vec3 out(0.0f);
    for (int i = 0; i <= k; i++) {
      out += (float)(bfGetBinomialCoefficient(k, i) * glm::pow(1 - t, k - i) *
                     glm::pow(t, i) * n) *
  ...
  glm::vec3 get_single_derivative_1_e(float t) {
    ...
    for (int i = 0; i <= n; i++) {
      out +=
          (float)((i - n * t) / (t * (1 - t)) * bfGetBinomialCoefficient(n, i) *
                  glm::pow(1 - t, n - i) * glm::pow(t, i)) *
          vertices[i];
    }
    return out;
  ...
  glm::vec3 get_single_derivative_2_e(float t) const {
    ...
    for (int i = 0; i <= n; i++) {
      if (t == 0) {
        out += (float)(n) / (n - 1) *
               (vertices[0] - 2.0f * vertices[1] + vertices[2]);
        continue;
      }
      if (t == 1) {
        out += (float)(n) / (n - 1) *
               (vertices[n] - 2.0f * vertices[n - 1] + vertices[n - 2]);
        continue;
      }

      out +=
          (float)((((i - n * t) * (i - n * t) - n * t * t - i * (1 - 2 * t)) /
                   (t * t * (1 - t) * (1 - t))) *
  ...
  /*
   * (CLASSIC_CALCULATION)
   * Returns Bezier curve witch is obtained by first derivative of current
   * Bezier curve
   *
   * Needed: nothing.
   *
  ...
  BfBezier get_derivative() {
    /*if (n == 1)
            throw std::runtime_error("Bezier derivative is undefined -> n will
       be < 1");*/
    ...
    for (int i = 0; i <= k; i++) {
      ...
      new_define_vertices[i] =
          static_cast<float>(n) * (this->vertices[i + 1] - this->vertices[i]);
    }
  ...
  BfBezier get_elevated_order() {
    ...
    for (int i = 0; i <= k; i++) {
      if (i == 0) {
        ...
        new_input_vertices[i] = ((float)(k - i) * this->vertices[i] +
                                 (float)i * this->vertices[i - 1]) /
                                (float)(k);
      }
    }
  ...
  /*
   * Returns 2 points (positions), each of which points to min and max point of
   * bounding box.
   *
   * Needed: count = number of points for search.
   *
  ...
  /*
   * Returns same Bezier curve alligned in way of setting start defining point
   * in (x=0, y=0, z=0), end defining point to input axe.
   *
   * Needed: allign_mode - one of defines: BF_ALLIGN_BEZIER_X,
   * BF_ALLIGN_BEZIER_Y, BF_ALLIGN_BEZIER_Z
   *
   */
  ...
  //		//o.vertices[i] = BfMatrix::multiply(rotate_matrix,
  //o.vertices[i]).get_vec3();
  //	}
  //
  //	glm::vec3 last_m;
  ...
  //				-asinf(last_m.y / sqrtf(last_m.y * last_m.y +
  //last_m.z * last_m.z))
  //			);
  //			BfQuaternion totalx = qx * BfQuaternion{ 1,0,0,0 };
  //			BfMatrix mx = totalx.get_rotation_matrix4();
  //
  //			last_m = BfMatrix::multiply(mx, glm::vec4(last_m.x,
  //last_m.y, last_m.z, 1.0f)).get_vec4();
  //
  //			BfQuaternion qy = BfQuaternion::get_rotation_quaternion(
  //				glm::normalize(glm::vec3(0.0f, 1.0f, 0.0f)),
  //				asinf(last_m.z / sqrtf(last_m.x * last_m.x +
  //last_m.z * last_m.z))
  //			);
  ...
  //
  //			for (size_t i = 0; i < this->vertices.size(); i++) {
  //				glm::vec4 l = { o.vertices[i].x,
  //o.vertices[i].y, o.vertices[i].z, 1.0f }; 				o.vertices[i] = (my * mx *
  //l).get_vec4();
  //			}
  ...
  //			}
  //			else {
  //				throw std::runtime_error("Input BF_AXIS_X can't
  //be processed with BF_PLANE_YZ");
  //			}
  ...
  //			BfQuaternion qy = BfQuaternion::get_rotation_quaternion(
  //				glm::normalize(glm::vec3(0.0f, 1.0f, 0.0f)),
  //				-asinf(last_m.x / sqrtf(last_m.x * last_m.x +
  //last_m.z * last_m.z))
  //			);
  ...
  //			BfQuaternion qx = BfQuaternion::get_rotation_quaternion(
  //				glm::normalize(glm::vec3(1.0f, 0.0f, 0.0f)),
  //				asinf(last_m.z / sqrtf(last_m.y * last_m.y +
  //last_m.z * last_m.z))
  //			);
  ...
  float get_average_increament_by_axis(uint32_t axis) {
    ...
    if (axis == BF_AXIS_X) {
      ...
      throw std::runtime_error(
          "Unknown input axis in "
          "BfBezier.get_average_increament_by_axis(uint32_t axis)");
    }
   ...
   *
   */
  std::vector<BfVertex3> get_handles_vertices(float length = 1.0f,
                                              glm::vec3 color = {1.0f, 0.0f,
                                                                 1.0f}) const {
    ...
    for (size_t i = 0; i < this->vertices.size(); i++) {
      ...
      std::copy(handle.get_vertices().begin(), handle.get_vertices().end(),
  ...
  std::vector<BfVertex3> get_carcass_vertices(glm::vec3 color = {1.0f, 0.0f,
                                                                 1.0f}) const {
  ...
  std::vector<glm::vec3> get_length_grad(std::vector<bool> is_working) {
    ...
    for (size_t p_index = 0; p_index < grad.size(); p_index++) {
      ...
      for (size_t j = 0; j < this->c_vertices.size(); j++) {
        ...
        for (size_t i = 0; i < this->vertices.size(); i++) {
          float basis_i =
              bfGetBinomialCoefficient(this->n, i) *
        ...
        glm::vec3 dl = {
            (d_j.x * dd_j.x) /
                (glm::sqrt(d_j.x * d_j.x + d_j.y * d_j.y + d_j.z * d_j.z)),
            (d_j.y * dd_j.y) /
                (glm::sqrt(d_j.x * d_j.x + d_j.y * d_j.y + d_j.z * d_j.z)),
            (d_j.z * dd_j.z) /
                (glm::sqrt(d_j.x * d_j.x + d_j.y * d_j.y + d_j.z * d_j.z)),
  ...
  static inline std::pair<glm::vec3, glm::vec3>
  get_bbox_by_iterator(std::vector<glm::vec3>::const_iterator it_begin,
  ...
  /*
   * Returns true if there are intesections between B1 and B2 else no
   *intersections;
   *
   * Needed:	intersections - reference to std::vector<glm::vec3> container to
   *store pos of intersections if there are more than 1; B1 - BfBezier curve 1;
   *			B2 - BfBezier curve 2;
   *
   */
  static inline bool
  get_intersections_simple(std::vector<glm::vec3> &intersections,
                           const BfBezier &B1, const BfBezier &B2) {

    if (B1.c_vertices.empty() || B2.c_vertices.empty()) {
      throw std::runtime_error(
          "intesections can't be found: no calculated points in one of curves");
    }
  ...
  /*
   * Returns true if there are intesections between Beizer curve and line else
   *no intersections;
   *
   * Needed:	intersections - reference to std::vector<glm::vec3> container to
   *store pos of intersections if there are more than 1; B - BfBezier curve; L -
   *BfLine;
   *
   */
  static inline bool
  get_intersections_simple(std::vector<glm::vec3> &intersections,
                           const BfBezier &B, const BfLine &L) {
  ...
  static inline bool
  get_itersections(std::vector<std::pair<glm::vec3, glm::vec3>> &inter,
                   BfBezier &b1, BfBezier &b2) {
    using it_pair = std::pair<std::vector<glm::vec3>::const_iterator,
    ...
    auto is_overlap = [&x_increment, &y_increment, &z_increment, &inter, &packs,
    ...
    curve_parts.push_back(std::make_pair(it_pair(vert1.begin(), vert1.end()),
    ...
    while (true) {
      ...
      if (size_for_current_iteration == count) {
        ...
        for (int i = 0; i < packs.size(); i++) {
          ...
          std::cout << "(" << packs[i].box1.first.x << ", "
                    << packs[i].box1.first.y << ", " << packs[i].box1.first.z
                    << "), ";
          std::cout << "(" << packs[i].box1.second.x << ", "
                    << packs[i].box1.second.y << ", " << packs[i].box1.second.z
                    << ")";
          std::cout << "),";

          std::cout << "(";
          std::cout << "(" << packs[i].box2.first.x << ", "
                    << packs[i].box2.first.y << ", " << packs[i].box2.first.z
                    << "), ";
          std::cout << "(" << packs[i].box2.second.x << ", "
                    << packs[i].box2.second.y << ", " << packs[i].box2.second.z
                    << ")";
          std::cout << "), ";
      ...
      for (auto it = curve_parts.begin(); it != curve_parts.end(); it++) {
        ...
        if (is_overlap(it->first, it->second)) {
          ...
          if (p1b_l >= 2) {
            p1b = it_pair(it->first.first,
                          it->first.first +
                              (it->first.second - it->first.first) / 2);
          }
          if (p1e_l >= 2) {
            p1e = it_pair(it->first.first +
                              (it->first.second - it->first.first) / 2,
                          it->first.second);
          }
          if (p2b_l >= 2) {
            p2b = it_pair(it->second.first,
                          it->second.first +
                              (it->second.second - it->second.first) / 2);
          }
          if (p2e_l >= 2) {
            p2e = it_pair(it->second.first +
                              (it->second.second - it->second.first) / 2,
                          it->second.second);
          }
...
private:
  ...
  glm::vec3 get_vertex_normal_by_curve(size_t index) const {
    return glm::cross(this->get_direction_normal(0.5f),
                      this->get_direction_tangent(0.5f));
  }
};
