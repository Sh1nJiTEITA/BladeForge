...
/**
 * @brief Вычисляет вершины дуги окружности в 3D-пространстве.
 *
 * Эта функция принимает три точки на окружности (начало, середину и конец дуги)
 * и генерирует аппроксимацию дуги, лежащей в плоскости, определенной этими
 * точками. Вершины дуги равномерно распределяются по углу между началом и
 * концом.
 *
 * @param p0 Первая точка дуги.
 * @param p1 Точка на середине дуги.
 * @param p2 Последняя точка дуги.
 * @param color Цвет всех вершин.
 * @param segments Количество отрезков (чем больше — тем плавнее дуга).
 * @return std::vector<BfVertex3> Вектор вершин с заданными позициями, нормалью
 * и цветом.
 */
std::vector< BfVertex3 > calcArcVertices(
    const glm::vec3& p0,
    const glm::vec3& p1,
    const glm::vec3& p2,
    uint32_t segments,
...
/**
 * @brief Вычисляет угол между двумя направлениями в 3D-пространстве.
 *
 * Функция принимает две точки, интерпретируемые как направляющие векторы от
 * начала координат, нормализует их и вычисляет угол между ними с помощью
 * скалярного произведения. Результатом является угол в **радианах**, лежащий в
 * диапазоне от 0 до π.
 *
 * @tparam P1 Тип первого аргумента, совместимый с BfVertex3.
 * @tparam P2 Тип второго аргумента, совместимый с BfVertex3.
 * @param _p1 Первая точка или вектор направления.
 * @param _p2 Вторая точка или вектор направления.
 * @return Угол между двумя нормализованными направлениями в радианах.
 *
 * @note В случае, если векторы почти коллинеарны или противоположны, результат
 * может быть чувствителен к погрешностям вычислений. Для устойчивости скалярное
 * произведение ограничивается диапазоном [-1.0, 1.0].
 */
template < typename P1, typename P2 >
float
angle(P1&& _p1, P2&& _p2)
{
   auto p1 = BfVertex3(std::forward< P1 >(_p1));
   auto p2 = BfVertex3(std::forward< P2 >(_p2));
   auto dir1 = glm::normalize(p1.pos);
   auto dir2 = glm::normalize(p2.pos);
