...
#ifdef BF_ALLOCATION_VIEWER
class BfAllocationViewer
{
   typedef struct
   {
      BfAllocatedBuffer *pBuffer;
      bool               is_alive;
   } BfQuickAllocInfo;

   static std::map<uint32_t, BfQuickAllocInfo> __map;

public:
   // BfAllocationViewer();
   // ~BfAllocationViewer();
   static void add(BfAllocatedBuffer *buffer);
   static void remove(BfAllocatedBuffer *buffer);

   static std::string print_info();
};
#endif
