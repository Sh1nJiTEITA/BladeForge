...
struct BfVertex3 {
  ...
  BfVertex3(glm::vec3 ipos)
      ...
      : pos{ipos} /*, color{icol}
                  , normals{inor}*/
        ,
        color{1.0f, 1.0f, 1.0f}, normals{0.0f, 0.0f, 0.0f}
  ...
  friend std::ostream &operator<<(std::ostream &os, const BfVertex3 &vert) {
    os << "BfVertex3( p(" << vert.pos.x << ", " << vert.pos.y << ", "
       ...
       << vert.color.b << "), n(" << vert.normals.x << ", " << vert.normals.y
       << ", " << vert.normals.z << ") )";
...
struct BfVertex4 {
  ...
  friend std::ostream &operator<<(std::ostream &os, const BfVertex4 &vert) {
    os << "BfVertex3( p(" << vert.pos.x << ", " << vert.pos.y << ", "
       << vert.pos.z << "), c(" << vert.color.r << ", " << vert.color.g << ", "
       << vert.color.b << "), n(" << vert.normal.x << ", " << vert.normal.y
       << ", " << vert.normal.z << ") obj_id(" << vert.obj_id << ')';
...
public:
  ...
  BfMeshHandler(uint32_t max_mesh_count) : __max_mesh_count{max_mesh_count} {
  ...
  uint32_t get_max_meshes_count() { return __max_mesh_count; }
  ...
  BfEvent allocate_mesh(VmaAllocator allocator, const uint32_t mesh_index,
                        BfeMeshType mesh_type) {
    ...
    bfCreateBuffer(&pMesh->vertex_buffer, allocator, vertex_size,
    ...
    bfCreateBuffer(&pMesh->index_buffer, allocator, index_size,
    ...
    BfSingleEvent event{};
    {
      ...
      if (pMesh->index_buffer.is_allocated &&
          pMesh->vertex_buffer.is_allocated) {
  ...
  BfEvent upload_mesh(const uint32_t mesh_index, BfMesh &input_mesh) {
    ...
    BfSingleEvent event{};
    {
      ...
      std::stringstream ss;
      ss << "Mesh(id = " << pMesh->id << ") Vertex buffer is correct;";
      event.info = ss.str();
      if (pMesh->vertices.size() <=
          bfGetMeshMaxSize(pMesh->type, sizeof(bfVertex))) {
  ...
  void load_mesh_to_buffers(VmaAllocator allocator, const uint32_t mesh_index) {
    ...
    {
      memcpy(vertex_data, pMesh->vertices.data(),
             sizeof(bfVertex) * pMesh->vertices.size());
    }
    ...
    {
      memcpy(index_data, pMesh->indices.data(),
             sizeof(uint32_t) * pMesh->indices.size());
    }
  ...
  void bind_mesh(VkCommandBuffer command_buffer, const uint32_t mesh_index) {
    ...
    vkCmdBindVertexBuffers(command_buffer, 0, 1, &pMesh->vertex_buffer.buffer,
                           offsets);
    vkCmdBindIndexBuffer(command_buffer, pMesh->index_buffer.buffer, 0,
  ...
  void draw_indexed(VkCommandBuffer command_buffer, const uint32_t mesh_index) {
    ...
    vkCmdDrawIndexed(command_buffer,
                     static_cast<uint32_t>(pMesh->indices.size()), 1, 0, 0,
                     mesh_index);
  }
};
//
...
static inline const std::map<BfeGeometrySetType, size_t>
    BfmGeometrySetTypeMaxElementsCount{
...
static inline const std::map<BfeGeometrySetType, size_t>
    BfmGeometrySetTypeMaxNumberOfVertices{
...
struct BfGeometrySet {
  ...
  void set_up(BfeGeometrySetType _type, size_t elements_count,
    ...
    indices.resize(BfmGeometrySetTypeMaxNumberOfVertices.at(
                       type) * // * max number of vertices in 1 curve
                               // BfmCurveTypeMaxElementsCount.at(type)
                   elements_count);
  }
  ...
  void draw_indexed(VkCommandBuffer command_buffer,
                    uint32_t obj_data_index_offset = 0) {
    VkDeviceSize offsets[] = {0};

    // BfExecutionTime::BeginTimeCut("VkBind");

    vkCmdBindVertexBuffers(command_buffer, 0, 1, &vertices_buffer.buffer,
                           offsets);
    vkCmdBindIndexBuffer(command_buffer, indices_buffer.buffer, 0,
                         VK_INDEX_TYPE_UINT16);
    ...
    for (size_t i = 0; i < this->ready_elements_count; i++) {
      ...
      vkCmdDrawIndexed(command_buffer, (uint32_t)pData->indices_count, 1,
                       (uint32_t)pData->index_offset, 0,
                       i + obj_data_index_offset);
  ...
  void update_object_data(VmaAllocation allocation) {
    ...
    {
      memcpy(pobjects_data, objects_data.data(),
             sizeof(BfObjectData) * objects_data.size());
    }
    vmaUnmapMemory(outside_allocator, allocation);
  }

  BfEvent add_data(const std::vector<BfVertex3> &_vertices,
                   const std::vector<uint32_t> &_indices,
                   const BfObjectData &obj_data) {
    return add_data(_vertices.data(), _vertices.size(), _indices.data(),
                    _indices.size(), &obj_data);

    /*BfSingleEvent event{};
    if (++ready_elements_count > allocated_elements_count) {
            event.type =
    BfEnSingleEventType::BF_SINGLE_EVENT_TYPE_GEOMETRY_SET_EVENT; event.action =
    BfEnActionType::BF_ACTION_TYPE_ALLOC_GEOMETRY_SET_FAILURE; event.info =
    ("All objects in geometry set with type = " + std::to_string(type) + "are
    ready, memory for additional object wasn't allocated"); return event;
    }
    ...
    if (ready_elements_count == 1) {
            ...
            BfGeometryData* pCurve_data_previous = &datas[ready_elements_count -
    2]; pCurve_data->index_offset = pCurve_data->indices_count *
    sizeof(BfVertex3) + pCurve_data_previous->index_offset;
    ...
    event.info = "New geometry vertices/indices/BfObjectData was added to
    BfGeometrySet = " + std::to_string(type); return event;*/
  }

  BfEvent add_data(const BfVertex3 *pVertices, size_t countVertices,
                   const uint32_t *pIndices, size_t countIndices,
                   const BfObjectData *obj_data) {
    BfSingleEvent event{};
    if (++ready_elements_count > allocated_elements_count) {
      event.type = BfEnSingleEventType::BF_SINGLE_EVENT_TYPE_GEOMETRY_SET_EVENT;
      ...
      event.info =
          ("All objects in geometry set with type = " + std::to_string(type) +
           "are ready, memory for additional object wasn't allocated");
      return event;
    }
    ...
    if (ready_elements_count == 1) {
      ...
      pCurve_data->index_offset =
          pCurve_data->indices_count * sizeof(BfVertex3) +
          pCurve_data_previous->index_offset;
  ...
  BfEvent allocate(BfeGeometrySetType type, size_t elements_count) {
    ...
    size_t max_vertices_size = // Sizes:
        ...
        BfmGeometrySetTypeMaxNumberOfVertices.at(
            type) * // * max number of vertices in 1 curve
        elements_count;
    // BfmCurveTypeMaxElementsCount.at(type);		// * max number of
    // curves

    // Create vertex buffer for loading to GPU
    bfCreateBuffer(&vertices_buffer, outside_allocator, max_vertices_size,
                   VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                   VMA_MEMORY_USAGE_CPU_TO_GPU);
    ...
    bfCreateBuffer(&indices_buffer, outside_allocator, max_indices_size,
                   VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                   VMA_MEMORY_USAGE_CPU_TO_GPU);
    ...
    BfSingleEvent event{};
    {
      ...
      std::stringstream ss;
      ss << "BfGeometrySet for type = " << this->type
         << ", Index buffer status: " << this->vertices_buffer.is_allocated
         << "; Vertex buffer status: " << this->indices_buffer.is_allocated;

      event.info = ss.str();

      if (this->indices_buffer.is_allocated &&
          this->vertices_buffer.is_allocated) {
        is_fully_allocated = true;
        event.action =
            BfEnActionType::BF_ACTION_TYPE_ALLOC_GEOMETRY_SET_SUCCESS;
        allocated_elements_count = elements_count;
      } else {
        is_fully_allocated = false;
        event.action =
            BfEnActionType::BF_ACTION_TYPE_ALLOC_GEOMETRY_SET_FAILURE;
      }
    }
    return BfEvent(event);
  }
  ...
  BfEvent deallocate() {
    vmaDestroyBuffer(outside_allocator, vertices_buffer.buffer,
                     vertices_buffer.allocation);
    vmaDestroyBuffer(outside_allocator, indices_buffer.buffer,
                     indices_buffer.allocation);

    BfSingleEvent event{};
    event.type =
        BfEnSingleEventType::BF_SINGLE_EVENT_TYPE_GEOMETRY_HOLDER_EVENT;
...
//
//
// static inline const std::map<BfeCurveType, size_t>
// BfmCurveTypeMaxElementsCount { 	{BF_CURVE_TYPE_BEZIER, 1000},
//	{BF_CURVE_TYPE_LINEAR, 10000}
// };
//
// static inline const std::map<BfeCurveType, size_t>
// BfmCurveTypeMaxNumberOfVertices{ 	{BF_CURVE_TYPE_BEZIER, 50},
...
//
//
//	void set_up(BfeCurveType _type, size_t elements_count, VmaAllocator
//allocator) { 		outside_allocator = allocator; 		type = _type; 		is_fully_allocated =
//false;
//
...
//		);
//
//		indices.resize(BfmCurveTypeMaxNumberOfVertices.at(type) *  // *
//max number of vertices in 1 curve
//			//BfmCurveTypeMaxElementsCount.at(type)
//			elements_count
//		);
//	}
//
//
//	void add_curve(std::vector<BfVertex3>& _vertices, std::vector<uint32_t>&
//_indices, BfObjectData obj_data) { 		if (++ready_elements_count >
//allocated_elements_count) { 			throw std::runtime_error("All curves in curve set
//with type = " + std::to_string(type) + "are ready, memory for additional curve
//wasn't allocated");
//		}
//
//		BfCurveData* pCurve_data = &curve_datas[ready_elements_count -
//1];
//
//		pCurve_data->vertices_count = _vertices.size();
//		pCurve_data->indices_count = _indices.size();
...
//		}
//		else {
//			BfCurveData* pCurve_data_previous =
//&curve_datas[ready_elements_count - 2]; 			pCurve_data->index_offset =
//pCurve_data->indices_count * sizeof(BfVertex3) +
//pCurve_data_previous->index_offset;
//
//			index_offset = 0;
...
//			for (size_t i = 0; i < _vertices.size(); i++) {
//				vertices[i + vertex_offset] = _vertices[i];
//				indices[i + index_offset] = _indices[i] +
//index_offset;
//			}
//		}
//		else {
...
//
//		void* vertex_data;
//		vmaMapMemory(outside_allocator, vertices_buffer.allocation,
//&vertex_data);
//		{
//			memcpy(vertex_data, vertices.data(), sizeof(BfVertex3) *
//vertices.size());
//		}
//		vmaUnmapMemory(outside_allocator, vertices_buffer.allocation);
//
//		void* index_data;
//		vmaMapMemory(outside_allocator, indices_buffer.allocation,
//&index_data);
//		{
//			memcpy(index_data, indices.data(), sizeof(uint32_t) *
//indices.size());
//		}
//		vmaUnmapMemory(outside_allocator, indices_buffer.allocation);
//	}
//
//	void draw_indexed(VkCommandBuffer command_buffer, uint32_t
//obj_data_index_offset = 0) { 		VkDeviceSize offsets[] = { 0 };
//		vkCmdBindVertexBuffers(command_buffer, 0, 1,
//&vertices_buffer.buffer, offsets); 		vkCmdBindIndexBuffer(command_buffer,
//indices_buffer.buffer, 0, VK_INDEX_TYPE_UINT16);
//
//		for (size_t i = 0; i < this->ready_elements_count; i++) {
//			BfCurveData* pData = &curve_datas[i];
//			vkCmdDrawIndexed(command_buffer,
//(uint32_t)pData->indices_count, 1, (uint32_t)pData->index_offset, 0, i +
//obj_data_index_offset);
//		}
//
//	}
//
...
//objects_data(this->ready_elements_count);
//
//		for (size_t i = 0; i < objects_data.size(); i++) {
...
//// Sizes: 			sizeof(BfVertex3) *
//// Of 1 vertex 			BfmCurveTypeMaxNumberOfVertices.at(type) *  // * max number of
//vertices in 1 curve 			elements_count;
//			//BfmCurveTypeMaxElementsCount.at(type);
...
//// Of 1 index 			BfmCurveTypeMaxNumberOfVertices.at(type) *  // * max number of
//vertices in 1 curve 			elements_count;
//			//BfmCurveTypeMaxElementsCount.at(type);
//// * max number of curves
//
...
//BfEnSingleEventType::BF_SINGLE_EVENT_TYPE_INITIALIZATION_EVENT;
//
//			std::stringstream ss; ss << "BfCurveSet for type = " <<
//this->type
//				<< ",Index buffer status:" <<
//this->vertices_buffer.is_allocated
//				<< "; Vertex buffer status:" <<
//this->indices_buffer.is_allocated;
//
//			event.info = ss.str();
//
//			if (this->indices_buffer.is_allocated &&
//this->vertices_buffer.is_allocated) { 				is_fully_allocated = true; 				event.action
//= BfEnActionType::BF_ACTION_TYPE_ALLOC_CURVE_SET_SUCCESS;
//				allocated_elements_count = elements_count;
//			}
//			else {
//				is_fully_allocated = false;
//				event.action =
//BfEnActionType::BF_ACTION_TYPE_ALLOC_CURVE_SET_FAILURE;
//			}
//		}
//		return BfEvent(event);
//	}
...
//	BfEvent deallocate() {
//		vmaDestroyBuffer(outside_allocator, vertices_buffer.buffer,
//vertices_buffer.allocation); 		vmaDestroyBuffer(outside_allocator,
//indices_buffer.buffer, indices_buffer.allocation);
//
//		BfSingleEvent event{};
//		event.type =
//BfEnSingleEventType::BF_SINGLE_EVENT_TYPE_CURVE_HOLDER_EVENT; 		event.action =
//BfEnActionType::BF_ACTION_TYPE_DEALLOC_CURVE_SET; 		event.info = " BfCurveSet
//for type = " + std::to_string(this->type); 		return BfEvent(event);
//	}
// };
//
//
