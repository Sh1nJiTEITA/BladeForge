...
template <class T> class BfMtxColumnIterator {
...
public:
  BfMtxColumnIterator(std::vector<T>::iterator b, const size_t n,
                      const size_t m);

  BfMtxColumnIterator<T> operator++();
};

template <class TI> class BfMatrixIterator {
...
public:
  ...
  BfMatrixIterator() : __ptr(nullptr), __n{1}, __m{1} {};
  ...
  friend bool operator==(const BfMatrixIterator &lhs,
                         const BfMatrixIterator &rhs) {
  ...
  friend BfMatrixIterator operator+(const BfMatrixIterator &it,
                                    difference_type n) {
  ...
  friend BfMatrixIterator operator+(difference_type n,
                                    const BfMatrixIterator &it) {
    return it + n;
  }
  ...
  friend difference_type operator-(const BfMatrixIterator &lhs,
                                   const BfMatrixIterator &rhs) {
...
template <class T> class BfMatrix2 {
...
public:
  ...
  BfMatrix2(size_t n, size_t m, T value) : __n{n}, __m{m} {
    __v.reserve(__n * __m);
  ...
  BfMatrixIterator<T> begin_row(size_t i) {
    return BfMatrixIterator<T>(&__v[i * __m], __n, __m,
                               BfMatrixIterator<T>::ROW_ITERATOR);
  }

  BfMatrixIterator<T> begin_col(size_t j) {
    return BfMatrixIterator<T>(&__v[j], __n, __m,
                               BfMatrixIterator<T>::COL_ITERATOR);
  }

  BfMatrixIterator<T> end_row(size_t i) {
    return BfMatrixIterator<T>(&(*this)(i, __m), __n, __m,
                               BfMatrixIterator<T>::ROW_ITERATOR);
  }

  BfMatrixIterator<T> end_col(size_t j) {
    return BfMatrixIterator<T>(&(*this)(__n, j), __n, __m,
                               BfMatrixIterator<T>::COL_ITERATOR);
  }

  BfMatrixIterator<T> rbegin_row(size_t i) {
    return BfMatrixIterator<T>(&(*this)(i, __m - 1), __n, __m,
                               BfMatrixIterator<T>::ROW_ITERATOR_R);
  }

  BfMatrixIterator<T> rbegin_col(size_t j) {
    return BfMatrixIterator<T>(&(*this)(__n - 1, j), __n, __m,
                               BfMatrixIterator<T>::COL_ITERATOR_R);
  }

  BfMatrixIterator<T> rend_row(size_t i) {
    return BfMatrixIterator<T>(&(*this)(i, 0), __n, __m,
                               BfMatrixIterator<T>::ROW_ITERATOR_R);
  }

  BfMatrixIterator<T> rend_col(size_t j) {
    return BfMatrixIterator<T>(&(*this)(0, j), __n, __m,
                               BfMatrixIterator<T>::COL_ITERATOR_R);
  }

  BfMatrixIterator<T> begin_dim_left(int i) {
    if (i == 0) {
      return BfMatrixIterator<T>(&(*__v.begin()), __n, __m,
                                 BfMatrixIterator<T>::LEFT_DIAGONAL_ITERATOR);
    } else if (i > 0) {
      if (i == __m)
        abort();
      return BfMatrixIterator<T>(&(*__v.begin()) + i, __n, __m,
                                 BfMatrixIterator<T>::LEFT_DIAGONAL_ITERATOR);
    } else {
      if (-i == __n)
        abort();
      return BfMatrixIterator<T>(&(*__v.begin()) - i * __m, __n, __m,
                                 BfMatrixIterator<T>::LEFT_DIAGONAL_ITERATOR);
    }
  }
  ...
  float det_LU() {
    ...
    float U_det = 1;
    for (auto it = LU.second.begin_dim_left(0); it != LU.second.end_dim_left(0);
         ++it) {
      U_det *= *it;
    }
    return L_det * U_det;
...
template <class T>
std::pair<BfMatrix2<T>, BfMatrix2<T>> luDecomposition(const BfMatrix2<T> &A) {
  size_t n = A.rows();
  BfMatrix2<T> L(n, n, 0);
  BfMatrix2<T> U = A;
  ...
  for (size_t k = 0; k < n - 1; ++k) {
    ...
    if (pivotRow != k) {
      std::swap_ranges(U.begin_row(k), U.end_row(k) + 1, U.begin_row(pivotRow));
      std::swap(P[k], P[pivotRow]);
      if (k > 0) {
        std::swap_ranges(L.begin_row(k), L.end_row(k) + 1,
