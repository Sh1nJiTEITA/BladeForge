...
#define IT_PAIR std::pair<std::vector<float>::iterator, std::vector<float>::iterator>
...
class BfMtxColumnIterator { 
    ...
    const size_t __m;
public:
    BfMtxColumnIterator(std::vector<T>::iterator b, const size_t n, const size_t m);
...
public:
	...
	switch (__t) { 
	    case ROW_ITERATOR: __ptr++; break;
	    case COL_ITERATOR: __ptr += __m; break; 
	    case ROW_ITERATOR_R: __ptr--; break;
	    case COL_ITERATOR_R: __ptr -= __m; break;
	    case LEFT_DIAGONAL_ITERATOR: __ptr += __m + 1; break;
	    case RIGHT_DIAGONAL_ITERATOR: __ptr += __m - 1; break;
	}
	return *this;
    }
	...
	switch (__t) { 
	    case ROW_ITERATOR: __ptr += n; break;
    ...
    friend bool operator==(const BfMatrixIterator& lhs, const BfMatrixIterator& rhs) {
    ...
    friend BfMatrixIterator operator+(const BfMatrixIterator& it, difference_type n) {
	BfMatrixIterator temp = it;
	temp += n;
	return temp;
    }

    friend BfMatrixIterator operator+(difference_type n, const BfMatrixIterator& it) {
	return it + n;
    }
    ...
    friend difference_type operator-(const BfMatrixIterator& lhs, const BfMatrixIterator& rhs) {
...
public:
    ...
    friend std::ostream& operator<<(std::ostream& os, const BfMatrix2<T>& matrix) {
	os << "BfMatrix2(";
...
std::pair<BfMatrix2<T>, BfMatrix2<T>> luDecomposition(const BfMatrix2<T>& A) {
    ...
    for (size_t k = 0; k < n - 1; ++k) {
        ...
        if (pivotRow != k) {
            ...
            if (k > 0) {
                std::swap_ranges(L.begin_row(k), L.end_row(k) + 1, L.begin_row(pivotRow));
            }
        }
