...
public:
   BfMatrix(size_t n, size_t m)
       : __n{n}, __m{m}, data{n, std::vector<float>(m, 0.0f)}
   {
   }
   ...
   static inline float det(BfMatrix& m0)
   {
      ...
      for (int i = 0; i < n; i++)
      {
         ...
         for (int j = i + 1; j < n; j++)
         {
            ...
            for (int k = 0; k < n; k++)
            {
               m0.data[j][k] = (num1 * m0.data[j][k]) - (num2 * temp[k]);
            }
...
struct BfQuaternion
{
   ...
   static inline BfQuaternion get_rotation_quaternion(const float x,
                                                      const float y,
                                                      const float z,
                                                      const float angle)
   {
   ...
   static inline BfQuaternion get_rotation_quaternion(const glm::vec3& axis,
                                                      const float      angle)
   {
      return BfQuaternion::get_rotation_quaternion(axis.x,
                                                   axis.y,
                                                   axis.z,
                                                   angle);
   }
