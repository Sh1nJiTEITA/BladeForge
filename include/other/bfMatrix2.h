...
public:
  ...
  BfVector(const std::vector<float> &v) : __data{v} {}
  ...
  template <size_t n> BfVector(glm::vec<n, float, glm::highp> v) {
    __data.reserve(n);
    fori(0, n) { __data.emplace_back(v[i]); }
  }

  template <size_t n> BfVector operator+(glm::vec<n, float, glm::highp> v) {
  ...
  template <size_t n> bool operator==(glm::vec<n, float, glm::highp> v) const {
    CHECK_DIMENSION_NUM(this->size(), n);
    bool is = true;
    fori(0, n) {
  ...
  BfVector(const std::vector<BfVertex3> &v) {
    ...
    forit(v) {
      fori(0, 3) { __data.emplace_back(it.pos[i]); }
    }
  }
  // #endif
  ...
  void emplace_back(float v) { __data.emplace_back(v); }

  BfVector get_erased_to(size_t n) const {
    CHECK_CAPACITY_NUM_LESS_EQ(this->size(), n);
    BfVector _v{};
    _v.reserve(n);
    fori(0, n) { _v.emplace_back((*this)[i]); }
    return _v;
  }
...
public:
  ...
  BfVec2(float x, float y) : BfVector{x, y}, x{__data[0]}, y{__data[1]} {}
...
public:
  BfMatrix(size_t n, size_t m, float v = 0.0f,
  ...
  BfMatrix get_triangular_form() const {
    ...
    fork_(0, this->rsize()) {
      ...
      fori(k + 1, this->rsize()) {
        BfVector mrow{_m[i]};
        forj(0, this->csize()) { mrow[j] -= wrow[j] * _m[i][k] / _m[k][k]; }
        std::swap(mrow, _m[i]);
      }
    }
    return _m * sign;
  ...
  BfMatrix get_transposed() const {
    ...
    fori(0, this->csize()) {
      ...
      forj(0, this->rsize()) { _m.rbegin()->emplace_back((*this)[j][i]); }
    }
    return _m;
  }
  ...
  BfMatrix get_inversed() const {
    ...
    fori(0, this->rsize()) {
      _m.push_back(BfVector());
      _m.rbegin()->reserve(this->csize());
      forj(0, this->csize()) { _m.rbegin()->emplace_back(adj[i][j] / det); }
    }
    return _m;
  }
  ...
  BfMatrix &inverse() {
    ...
    fori(0, this->rsize()) {
      forj(0, this->csize()) { (*this)[i][j] = adj[i][j] / det; }
    }
    return *this;
  }
  ...
  BfVector to_bfvec() const {
    ...
    fori(0, this->rsize()) {
      forj(0, this->csize()) { _v.emplace_back((*this)[i][j]); }
    }
    return _v;
  }
  ...
  BfMatrix operator>>(const BfMatrix &v) {
    ...
    fori(0, this->rsize()) {
      ...
      forj(0, v.csize()) {
        float c = 0.0f;
        fork_(0, v.rsize()) { c += (*this)[i][k] * v[k][j]; }
...
public:
  ...
  BfLineProp(float k, float b) : __k{k}, __b{b} {}
